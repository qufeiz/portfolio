...
**Brought in a fresh set of product screenshots that would make a strong walkthrough? REQUEST a video, don't render it.** You cannot spawn another subagent. Append an Open row to `state/video-queue.md` (a small video-spec: title + ordered shots `image|fit|caption`, true to the product, no em-dashes + target page/output) and flag it in your report; the LEAD spawns `make-video` to fulfill it OFFLINE and embed it. See the ref's "Optional — REQUEST a video".
