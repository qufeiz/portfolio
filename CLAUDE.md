...
- **TreAxe** — co-built/co-owned **with a collaborator**; never imply sole authorship. Live product: www.treaxe.io. ⚠️ The previously-published demo login (`staging-test@treaxe-test.com / TreAxeDemo2026!`) was reported **failing in production** ("Invalid email or password"), so it's been pulled from the live page — hands-on demo access is now gated **on request** (mailto), with the walkthrough video as the always-working tour. Minting/confirming a working demo login lives on the (co-owned) TreAxe side, NOT this repo; do not republish static creds until a working pair is verified. Gallery uses TreAxe's OWN official screenshots from `~/Projects/TreAxe/docs/features/*/images/` and `docs/product/images/` (prefer these over re-captures).
