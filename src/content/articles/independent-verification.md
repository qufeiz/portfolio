...
:::note{title="The bug in one line"}
A "Send to Client" button shipped to production for months with a one-line handler that only flipped a status field. The journey test checked the status pill, which was wired straight to that field, so it stayed green the whole time. No email was ever sent.
:::
